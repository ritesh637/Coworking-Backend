...
const generateInvoiceId = async () => {
  ...
  const month = currentDate
    ...
    .toUpperCase(); // 'APR'
  const day = currentDate.getDate().toString().padStart(2, "0"); // e.g., '29'

  // Regex to match only current month+year invoices
  const regex = new RegExp(`^PRO-${month}\\d{2}${year}-\\d+$`);
  ...
  let counter = 1;
  if (lastPayment) {
    const lastInvoiceId = lastPayment.invoiceId;
    const lastCounter = parseInt(lastInvoiceId.split("-")[2], 10);
    counter = lastCounter + 1;
  }

  const formattedCounter = counter.toString().padStart(2, "0");

  return `PRO-${month}${day}${year}-${formattedCounter}`;
};
...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    const payment = new Payment({
      ...
      invoiceId, // Use generated invoiceId
    });
